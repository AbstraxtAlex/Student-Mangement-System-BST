...
struct BST {
		...
		void inOrderPrintReverse(BTNode* );//to get data in descending by visiting right first
		void inOrderPrintReverseToFile(BTNode* , ofstream&);
		bool printPath();
		void printPathRootToLeaf(BTNode*, int[], int);//store the data from visiting root to leaf
		bool deepestNodes();
		void printDeepest(BTNode*, int[], int);
		bool printLevelNodes();
		void printLevelNodes2(BTNode*, int[], int, int);
		bool printGivenLevel(BTNode* , int );
		bool CloneSubtree(BST,type );
		BTNode* search(type item);//search for same id in tree
		BTNode* cloneSubtree(BTNode* node);//copy all the son and grandson from the root chosen
